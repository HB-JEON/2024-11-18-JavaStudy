 ...
 *  127 page
 *   1. 주의점
 *     1) static 메소드에서는 인스턴스 변서ㅜ, 인스턴스메소드를 사용할 수 없다
 *        > 사용 시에 객체 생성 ㅎ에 사용 가능
 *     2) static 메소드, static 변수는 this 사용 불가
 *     3) static 블록은 static 변수만 초기화 가능
 *     4) 인스턴스 메소드에서는 인스턴스 변수, static 사용 가능
 *     5) 초기화 > 생성자를 이용하는 이유
 *               ---- this, 인스턴스, static 
 *       
 ...
 *      			 생성자, 메소드
 *      -------------------------------------------------------------------------
 *      protected 	멤버변수, 생성자			같은 폴더(패키지)			다른 폴더 사용 불가능
 *      			메소드				또는 상속 받은 클래스
 *      								 > 다른 폴더에 있는 경우
 *      -------------------------------------------------------------------------
 *      default		클래스, 멤버변수			같은 폴더에서만 접근 가능		다른 폴더 사용 불가능
 *      			생성자, 메소드
 *      -------------------------------------------------------------------------
 ...
 *             메소드의 역할
 *              > 다른 클래스와 통신(메세지) > public
 *      생성자 - public
 *      멤버변수 - private(데이터 보호)를 주로 사용 > 은닉화
 *               메모리에서 출력 X
 *               통신 방법
 *                > 변수 - 쓰기(메모리 저장), 읽기(메모리에서 데이터 가져오기)
 *                        메소드 처리 - getter/setter
 *      지역변수 - 접근지정어 필요 X (메소드 안에서만 사용 가능)
 *               상수(final)
 *    *** 객체지향의 3대 특성(권장)
 *       1. 캡슐화 - 외부로부터 데이터 보호
 *                 데이터 손상(id,pwd...)
...
class Member
{
	private String id; // Member 클래스 안에서만 사용 가능 / RED
