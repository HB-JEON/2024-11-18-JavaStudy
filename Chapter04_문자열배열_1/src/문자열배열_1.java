 ...
 *   메소드 > 클래스
 *   클래스 > 라이브러리
 *  처리하는 기능이 많을 경우 소스 코딩이 많아짐
 *  
 *  1) 문자열 > 웹/윈도우 프로그램 대부분 문자열로 구성
 *            가장 많이 사용되는 데이터
 *            문자를 여러개를 모아서 저장
 *            --------- char[] > String
 *  2) String : 일반 데이터형 / 클래스형
 *              사용법 - 일반 데이터형(가장 많이 사용)
 *                     String 변수명="";
 *                     클래스형
 *                     String 변수명=new String ("저장 데이터","UTF-8")
 *              한글 변환 (브라우저 > char > 1yte)
 *  3) String의 디폴트(null) (주소가 없는 상태)
 *     > 모든 클래스의 디폴트 > NullpointerExeption
 *  4) String의 기능(메소드)
 *     ** String, Math, Scanner, System = Final class **
 *                                          | 변경 불가능
 *       ** java.long > iport 생략 가능          있는 그대로 사용
 *                                            표준화
 *     - 변환      
 *        toUpperCase() - 대문자 변환
 *         > 메소드
 ...
 *             String substring(int begin, int end) // end-1까지
 ...
 *        endsWith() - 끝나는 문자열이 시작하는 경우
 *           > boolean endsWith(String s)
 *     ** contains() - 포함된 문자열
 *           > boolean contains(String s)
 *     
 *     - 위치 찾기
 *     ** intdexOf() - 처음부터 입력 : (문자/문자열)
 *           > int indexOf(char c)
...
public class 문자열배열_1 {
	...
	public static void main(String[] args) {
		...
		for(String n:names)
		{
			// equalsIgnoreCase - 대소문자 구분 없이 처리
